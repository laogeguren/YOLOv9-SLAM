...
int main(int argc, char **argv)
{
    ...
    cout<<"关联文件路径："<<strAssociationFilename <<endl;
 ...
 //------------------------------------------------------------------------------------   
    for(int ni=0; ni<nImages; ni++)
    {
        ...
        */
        cout<<endl<<endl;
        cout<<"*****彩图:"<<vstrImageFilenamesRGB[ni]<<endl;
        cout<<"*****时间戳："<<std::to_string(vTimestamps[ni])<<endl;
        string image_path=vstrImageFilenamesRGB[ni];
        
        image_path=image_path.substr(4);
      	image_path=image_path.substr(0,image_path.length()-4);
        
        cout<<"yolod读取文件名："<<image_path<<endl;
        
        
        string strPathToDetectionResult = argv[5] + image_path + ".txt";//读取detect_result
	
	cout<<"yolod文件路径："<<strPathToDetectionResult<<endl;
	cout<<endl<<endl;
	
        LoadBoundingBox(strPathToDetectionResult, detect_result);
     if(0){   if (detect_result.empty()){
  ...
  //-----------------------------------------------------------------------------------      
    ...
    SLAM.SaveTrajectoryTUM("/home/zs/YOLOV9_ORB_SLAM/runs2/result003_walking_halfsphere/myslam.txt");

    SLAM.SaveKeyFrameTrajectoryTUM("/home/zs/YOLOV9_ORB_SLAM/runs2/result003_walking_halfsphere/KeyFrameTrajectory001.txt");   
...
void LoadBoundingBox(const string& strPathToDetectionResult, vector<std::pair<vector<double>, int>>& detect_result){
	...
	//cout<<"yolo结果文件："<<strPathToDetectionResult<<endl;
