...
public:
    ...
    bool IsInStaticDynamic(const int& i);
    

public:

cv::Mat mImGray;
float GetDepth(const cv::Point2f &pt);
    cv::Mat mLastDepth; // 上一帧深度图（原代码已使用，需确保存在）

    cv::Mat mLastPose, mCurrentPose; // 位姿矩阵（原代码已使用，需确保存在）
