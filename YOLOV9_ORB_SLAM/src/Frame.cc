     ...
     mTimeStamp(timeStamp), mK(K.clone()),mDistCoef(distCoef.clone()), mbf(bf), mThDepth(thDepth)
{
    ...
    if(mvKeys.empty())
     ...
     Timer timer;
    timer.Start("Feature Delete");  // 开始计时，标签为“特征删除”
    //*******直接检验特征点是否在动态物体的框内
    // cout << "mvKeys.size() is " << mvKeys.size() << endl;
    for (int k=0; k<mvKeys.size(); ++k){
    
    
        bool isInDyn = IsInDynamic(k);
        bool isInStat = IsInStatic(k);
        bool isStaticDyn = IsInStaticDynamic(k); 
    /******************
        if (isInDyn == true && isInStat == false && isStaticDyn == false){ 
        //在动态物体框内,但不在静态物体框内
            vbInDynamic_mvKeys.push_back(true);
            // cout << "true" << endl;
            mvKeys[k] = cv::KeyPoint(-100,-100,-1);
        }
        if (isInDyn == false && isInStat == true && isStaticDyn == false){ 
                    vbInDynamic_mvKeys.push_back(false);
            // cout << "false" << endl;
        }
    
        if (isInDyn == true && isInStat == true && isStaticDyn == true ){ 
       //在动态物体框内,在静态物体框内,大于阈值，判断为动态
            vbInDynamic_mvKeys.push_back(true);
            // cout << "true" << endl;
        mvKeys[k] = cv::KeyPoint(-100,-100,-1);
        
        }
        if (isInDyn == true && isInStat == true && isStaticDyn)== false ){ 
       //在动态物体框内,在静态物体框内,小于阈值，判断为动态
            vbInDynamic_mvKeys.push_back(true);
            // cout << "true" << endl;
            ...
            ********************/
             ...
             if (isInDyn && !isInStat && !isStaticDyn) { 
        //在动态物体框内,但不在静态物体框内
            vbInDynamic_mvKeys.push_back(true);
            mvKeys[k] = cv::KeyPoint(-100,-100,-1);
        }
        else if (!isInDyn && isInStat && !isStaticDyn) { 
            vbInDynamic_mvKeys.push_back(false);
        }
        else if (isInDyn && isInStat && isStaticDyn) { 
       //在动态物体框内,在静态物体框内,大于阈值，判断为动态
...
void Frame::ExtractORB(int flag, const cv::Mat &im)
{
    ...
    Timer timer;
    timer.Start("Feature Extraction");  // 开始计时，标签为“特征提取”
    // 判断是左图还是右图
    if(flag==0)
...
cv::Mat Frame::UnprojectStereo(const int &i)
{
    ...
    bool Frame::IsInBox(const int& i, int& box_id) {//判断kp是否在bounding_box_(txt读入的数据)内. objects_cur_存储帧中的object,由frame.cc引入
        ...
        for (int k = 0; k < objects_cur_.size(); ++k) {//遍历所有物体
            ...
            double right = box[2];
            double bottom = box[3];
           if (kp_u > left + 1 && kp_u < right - 1
                    && kp_v > top + 1 && kp_v < bottom -1) {
                in_box = true;
                box_id = k;
                break;
            }
    ...
    bool Frame::IsInDynamic(const int& i) {//判断kp是否在bounding_box_(txt读入的数据)内. objects_cur_存储帧中的object,由frame.cc引入
        ...
        int obj_class = objects_cur_[k]->ndetect_class;
            ...
            if (obj_class == 3){
                ...
                if (kp_u > left + 5 && kp_u < right - 5
                    && kp_v > top + 5 && kp_v < bottom - 5) {
                    in_dynamic = true;
                }
            }
        }
 ...
 bool Frame::IsInStaticDynamic(const int& i) {
    // 安全检查
    if (i < 0 || i >= mvKeys.size()) {
        std::cerr << "Error: Feature point index out of range" << std::endl;
        return false;
    }
    ...
    float kp_u = kp.pt.x;
    float kp_v = kp.pt.y;
    bool in_staticdynamic = false;
    
    
  /***********  
    // 检查是否同时位于动态和静态物体的检测框内
    bool in_class3 = false;
    bool in_class1_or_2 = false;
    ...
    for (int k = 0; k < objects_cur_.size(); ++k) {
        ...
        if (kp_u > left + 5 && kp_u < right - 5 && 
            ...
            if (obj_class == 3) {
                in_class3 = true;
            } else if (obj_class == 1 || obj_class == 2) {
                in_class1_or_2 = true;
            }
        }
    }
    ...
    // 如果不在重合区域，直接返回false
    if (!(in_class3 && in_class1_or_2)) {
        return false;
    }
    *******************/
    
    Timer timer;
    timer.Start("Feature Filter");  // 开始计时，标签为“特征筛选”
    for (size_t k = 0; k < objects_cur_.size(); ++k) {//遍历所有物体
        int obj_class = objects_cur_[k]->ndetect_class;

            if (obj_class == 1||obj_class == 3){
    ...
    // 使用当前帧的相机内参
    float fx_local = mK.at<float>(0,0);
    float fy_local = mK.at<float>(1,1);
    float cx_local = mK.at<float>(0,2);
    float cy_local = mK.at<float>(1,2);
    
    for (int m = max(0, (int)kp_v - sample_radius); 
         m < min(mImGray.rows, (int)kp_v + sample_radius); 
         m += sample_step) {
        for (int n = max(0, (int)kp_u - sample_radius); 
             n < min(mImGray.cols, (int)kp_u + sample_radius); 
             n += sample_step) {
            
            //float d = GetDepth(cv::Point2f(n, m));
            //float d = mvDepth.ptr<float>(m)[n];
            float d = mvDepth[i];
            if (d < 0.01f || d > 10.0f) continue;
            
            // 计算当前帧的3D点（相机坐标系）
            pcl::PointXYZ p_current;
            p_current.z = d;
            p_current.x = (n - cx_local) * p_current.z / fx_local;
            p_current.y = (m - cy_local) * p_current.z / fy_local;
            
            // 与上一帧比较（如果上一帧数据可用）
            if (!mLastDepth.empty() && mLastDepth.rows == mImGray.rows && 
                mLastDepth.cols == mImGray.cols && !mLastPose.empty() && !mCurrentPose.empty()) {
                
                float d_prev = mLastDepth.ptr<float>(m)[n];
                if (d_prev >= 0.01f && d_prev <= 10.0f) {
                    // 计算上一帧的3D点（相机坐标系）
                    pcl::PointXYZ p_previous;
                    p_previous.z = d_prev;
                    p_previous.x = (n - cx_local) * p_previous.z / fx_local;
                    p_previous.y = (m - cy_local) * p_previous.z / fy_local;
                    
                    // 转换到世界坐标系
                    cv::Mat P_prev = (cv::Mat_<float>(3,1) << p_previous.x, p_previous.y, p_previous.z);
                    cv::Mat P_curr = (cv::Mat_<float>(3,1) << p_current.x, p_current.y, p_current.z);
                    
                    cv::Mat P_world_prev = mLastPose * P_prev;
                    cv::Mat P_world_curr = mCurrentPose * P_curr;
                    
                    // 计算位移
                    double displacement = cv::norm(P_world_curr - P_world_prev);
      
                    if (displacement > DISPLACEMENT_THRESHOLD) {
                        in_staticdynamic= true;
                        
                    }
                    if (displacement < DISPLACEMENT_THRESHOLD) {
                    in_staticdynamic = false;
