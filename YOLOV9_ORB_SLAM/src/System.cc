...
Timer timer;
    timer.Start("System Initialization");  // 系统初始化计时
							...
							const double &timestamp)	//时间戳
{
    ...
    // Check mode change
    {
    	// 互斥锁，以免mbActivateLocalizationMode、mbDeactivateLocalizationMode被修改
...
// cv::Mat System::TrackRGBDwithDetect(const cv::Mat &im, const cv::Mat &depthmap, const double &timestamp, const vector<std::pair<vector<double>, int>>& detect_result)
// {
// 	//判断输入数据类型是否合法
//     if(mSensor!=RGBD)
//     {
...
//     }
//     //****
//     vector<vector<double>> vfirst;
//     vector<int> vsecond;
//     vector<vector<double>> vperson_box;
//     vector<int> vperson_class;
...
//         vfirst.push_back(detect_result[k].first);////把所有物体检测存在了vfirst和vsecond中
//         vsecond.push_back(detect_result[k].second);

//         if (detect_result[k].second == 3){ //把人的检测存在了vperson_box和vperson_class中.  若关注其他类别则再加
//             vperson_box.push_back(detect_result[k].first);
//             vperson_class.push_back(detect_result[k].second);
//         }

//     }
//     // cout << "当前帧的人的数目: " << vperson_class.size() << endl;

//     std::shared_ptr<Object> obj = make_shared<Object>(vperson_box,vperson_class);//obj储存了人的检测框 
...
//同理，输入为单目图像时的追踪器接口
cv::Mat System::TrackMonocular(const cv::Mat &im, const double &timestamp)//**在进行tracking前进行一些判断及一些状态设定
{
    if(mSensor!=MONOCULAR)
    {
...
void System::SaveTrajectoryTUM(const string &filename)
{
    ...
    //所有帧对应的时间戳列表
    list<long double>::iterator lT = mpTracker->mlFrameTimes.begin();
    //每帧的追踪状态组成的列表       
...
void System::SaveKeyFrameTrajectoryEuroc(const string &filename)
{
    ...
    for(size_t i=0; i<vpKFs.size(); i++)
    {
       ...
       // pKF->SetPose(pKF->GetPose()*Two);
        ...
        cv::Mat t = pKF->GetCameraCenter();
        //按照给定的格式输出到文件中
        f <<setprecision(9) <<pKF->mTimeStamp << " " << t.at<float>(0) << " " << t.at<float>(1) << " " << t.at<float>(2)
          << " " << q[0] << " " << q[1] << " " << q[2] << " " << q[3] << endl;
